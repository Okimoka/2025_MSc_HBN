...
def _read_iview_samples(sample_fname: str):
    """Parse the *_Samples.txt file and return Raw, times, inline messages, and metrics."""

    #TODO read in using csv.reader, like _events file
    ...
    time_idx = 0  #header.index("Time")
    type_idx = header.index("Type")

    # numeric columns: all except Type
    num_idx = [i for i in range(len(header)) if i != type_idx]
    # channel names: all numeric columns except Time
    ch_names = [header[i] for i in num_idx if i != time_idx]

    data_cols = [[] for _ in num_idx]
    inline_msgs: list[tuple[float, str]] = []
    et_broken_values = 0
    ...
    for ln in lines[header_idx + 1:]:
        ...
        # broken values heuristic
        # if all these cols are 0.00, the sample is worthless
        if parts[3:11] == ["0.00"] * 8:
            et_broken_values += 1

        # inline MSG row, looks like "2655102177\tMSG\t1\t# Message: 20"
        # readable: "2655102177 MSG 1 # Message: 20"
        if len(parts) == 4:
        ...
        for j, ci in enumerate(num_idx):
            v = parts[ci] if ci < len(parts) else ""
    ...
    # time is always the first numeric column, so skip nan_cols[0] when aligning to ch_names
    ch_names = [nm for nm, is_nan in zip(ch_names, nan_cols[1:]) if not is_nan]

    # first column is time in µs
    times = np.asarray(data_cols[0], float)
    data = np.vstack([np.asarray(c, float) for c in data_cols[1:]])

    # estimated sampling characteristics
    # when just using sfreq, some annotations may fall outside raw time range
    ...
    metrics = {
        "sfreq_header": sfreq, # from header line
        "sfreq_est": sfreq_est, # from median dt
        "broken_value_rows": et_broken_values
    }

    return raw_et, times, inline_msgs, metrics

# Parse *_Events.txt
def _read_iview_events(event_fname: str, t0: float):
    
    #t0 is taken from the _Samples file. There is no real replacement in the _Events file.
    #Just taking the eariliest timestamp from  _Events  does not always work (e.g. NDARKM199DXW)

    ann_on: list[float] = [] # onset
    ann_du: list[float] = [] # duration
    ann_ds: list[str] = [] # description
    ann_ex: list[dict | None] = [] # extras (since mne 1.10.0)

    et_unknown_events = 0

    # TODO: use table headers instead of hardcoded len(parts) checks
    with open(event_fname, "r", encoding="utf-8", errors="ignore") as ef:
        for parts in csv.reader(ef, delimiter="\t"):
            ...
            # readable:  "UserEvent 1 5 2547585001 # Message: 14"
            ...
            # Looks like "Fixation L\t1\t1\t2524661981\t2524811929\t149948\t1077.17\t..."
            # readable (no commas) "Fixation L, 1, 1, 2524661981, 2524811929, 149948, 1077.17, 697.01, 29, 17, -1, 12.61, 12.61
            elif len(parts) >= 5 and (
                kind.startswith("Fixation")
                or kind.startswith("Saccade")
                or kind.startswith("Blink")
            ):
    ...
    metrics = {
        "unknown_events": et_unknown_events,
    }

    return ann_on, ann_du, ann_ds, ann_ex, metrics
...
cfg is needed only for sync_eventtype_regex_et
"""
def read_raw_iview(
    sample_fname: str,
    event_fname: str | None = None,
    cfg: SimpleNamespace | None = None,
):

    # parse samples file and build Raw
    #TODO use unified format for both functions to return annotations
    ...
    raw_et, times, inline_msgs_samples, metrics = _read_iview_samples(sample_fname)
    t0 = times[0]

    ann_on: list[float] = []
    ann_du: list[float] = []
    ann_ds: list[str] = []
    ann_ex: list[dict | None] = []
    eventfile_matches: bool | list[bool] = False
    metrics.setdefault("unknown_events", 0)

    if event_fname and os.path.exists(event_fname):
        ann_on, ann_du, ann_ds, ann_ex, event_metrics = _read_iview_events(event_fname, t0)
        metrics.update(event_metrics)

        # check whether inline_msgs (from _Samples) matches the events from _Events
        regex = getattr(cfg, "sync_eventtype_regex_et", None) if cfg is not None else None
        eventfile_matches = [
            _samples_events_matches(ann_on, ann_ds, inline_msgs_samples, t0),
            _samples_events_matches(ann_on, ann_ds, inline_msgs_samples, t0, regex),
        ]

    else:
        # Fallback: build annotations from inline messages in *_Samples
        logger.info(**gen_log_kwargs(message="SMI _Events.txt not found, using Events from _Samples.txt"))
        for t_us, desc in inline_msgs_samples:
            ann_on.append((t_us - t0) / 1e6)
            ann_du.append(0.0)
            ann_ds.append(desc)
            ann_ex.append(None)

    metrics["eventfile_matches"] = eventfile_matches
 ...
 *,
    ...
    metrics = dict(
        ...
        max_gap_between_shared_events_samples=np.nan,
    ...
    for idx, (run, raw_fname,et_fname) in enumerate(zip(cfg.runs, raw_fnames,et_fnames)):
        ...
        if et_format == '.edf':
            ...
            raw_et, iview_metrics = read_raw_iview(str(et_fname), event_fname, cfg)
            #et_eventfile_matches, et_unknown_events, sfreq_est, et_broken_values
            metrics["et_eventfile_matches"] = iview_metrics["eventfile_matches"][0]
            metrics["et_eventfile_matches_filtered"] = iview_metrics["eventfile_matches"][1]
            metrics["et_unknown_events"] = iview_metrics["unknown_events"]
            metrics["et_sfreq_est"] = iview_metrics["sfreq_est"]
            metrics["et_nan_values"] = iview_metrics["broken_value_rows"]
            print("BROKEN " + str(iview_metrics["broken_value_rows"]))
        else:
    ...
    if len(raw_onsets):
        ...
        # longest gap without any shared events (between two shared events), in EEG samples
        onset_samples = np.sort(np.array(raw_onsets) * float(raw.info["sfreq"]))
        if onset_samples.size > 1:
            gaps = np.diff(onset_samples)  # distances between consecutive shared events in samples
            metrics["max_gap_between_shared_events_samples"] = float(gaps.max())
